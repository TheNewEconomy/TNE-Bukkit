...
public class AuctionStartCommand extends TNECommand {
  ...
  public boolean execute(CommandSender sender, String command, String[] arguments) {
    ...
    Integer amount = 1;
    BigDecimal start = TNE.instance().api().getBigDecimal("Core.Auctions.MinStart", world, IDFinder.getID(player).toString());
    BigDecimal increment = TNE.instance().api().getBigDecimal("Core.Auctions.MinIncrement", world, IDFinder.getID(player).toString());
    ...
    if(start.compareTo(TNE.instance().api().getBigDecimal("Core.Auctions.MaxStart", world, IDFinder.getID(player))) > 0) {
      start = TNE.instance().api().getBigDecimal("Core.Auctions.MaxStart", world, IDFinder.getID(player));
    }

    if(increment.compareTo(TNE.instance().api().getBigDecimal("Core.Auctions.MaxIncrement", world, IDFinder.getID(player))) > 0) {
      increment = TNE.instance().api().getBigDecimal("Core.Auctions.MaxIncrement", world, IDFinder.getID(player));
    }
