 ...
 * All rights reserved.
 **/
public class Alpha5_6 extends Version {
  @Override
  ...
  public void update(double version, String type) {
    ...
    if(version == 5.5) {
      ...
      sql().executeUpdate("ALTER TABLE `" + table + "` ADD COLUMN `last_online` BIGINT AFTER `joinedDate`");
    }
  }
  ...
  public Collection<Account> loadAccounts() {
    ...
    try {
      ...
      while (sql().results(accountIndex).next()) {
        ...
        account.setLastOnline(sql().results(accountIndex).getLong("last_online"));
  ...
  public void saveAccount(Account acc) {
    if(!TNE.instance().saveManager.type.equalsIgnoreCase("flatfile")) {
      String table = prefix + "_USERS";
      sql().executePreparedUpdate("INSERT INTO `" + table + "` (uuid, acc_pin, inventory_credits, command_credits, joinedDate, last_online, accountnumber, accountstatus, account_special) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)" +
              " ON DUPLICATE KEY UPDATE acc_pin = ?, inventory_credits = ?, command_credits = ?, joinedDate = ?, last_online = ?, accountnumber = ?, accountstatus = ?, account_special = ?",
          new Object[]{
              acc.getUid().toString(),
              acc.getPin(),
              acc.creditsToString(),
  ...
  public void loadFlat(File file) {
    ...
    while(it.hasNext()) {
      ...
      account.setLastOnline((Long)info.getData("last_online"));
  ...
  public void saveFlat(File file) {
    ...
    while(accIT.hasNext()) {
      ...
      info.addData("last_online", acc.getLastOnline());
