...
import net.tnemc.signs.signs.item.ItemEntry;
...
public class ItemSign extends TNESign {
  ...
  public void addOffer(Player player, String[] lines) {
    TNE.debug("ItemSign addOffer lines: " + lines.toString());
    ...
    if(offers.size() > 0 && !TNE.instance().api().getBoolean("Signs.Item.Multiple", world, IDFinder.getID(player))) {
      new Message("Messages.SignShop.NoMultiple").translate(world, player);
      return;
    }

    if(offers.size() == TNE.instance().api().getInteger("Signs.Item.MaxOffers", world, owner.toString())) {
    ...
    if(!itemType.equals(Material.AIR)) {
      ...
      if(!MaterialHelper.getMaterial(separated[0]).equals(Material.AIR)) {
        ...
        if(!material.equals(Material.AIR)) {
          TNE.debug("Adding trade offer for material " + material.name().toLowerCase());
        ...
        for(String s : separated) {
          ...
          TNE.debug("Line 2: " + lines[2]);
          TNE.debug("Value: " + split[0]);
          ...
          switch(split[1].toUpperCase().trim()) {
            case "B":
              TNE.debug("Added buy value of " + value);
              buy = value;
              break;
            case "S":
              TNE.debug("Added sell value of " + value);
              sell = value;
              break;
            default:
              break;
      ...
      TNE.debug("Added offer for " + item.getType().toString());
      ...
      SignsModule.manager().getSigns().put(location, this);
    }
  }
  ...
  private void updateLines(Sign s) {
    ...
    TNE.debug("ItemSign.updateLines Offer: " + currentOffer);
    ...
    if(!entry.getTrade().getType().equals(Material.AIR)) {
      TNE.debug("Added trade values");
      buy = MaterialHelper.getShopName(entry.getTrade().getType());
      if(entry.getTrade().getAmount() > 1) {
        buy = buy + ":" + entry.getTrade().getAmount();
      }
    } else {
      TNE.debug("Added sell/buy values");
      TNE.debug("Buy Double: " + entry.getBuy().doubleValue());
      TNE.debug("Sell Double: " + entry.getSell().doubleValue());
    ...
    String author = (entry.admin)? ChatColor.RED + "Admin" : IDFinder.getOffline(owner).getName();
    TNE.debug("Buy line: " + buy);
    s.setLine(2, buy);
    s.setLine(3, author);
    s.update(true);
  }
  ...
  private boolean changeStock(Player player, Material material, int amount, boolean add) {
    ...
    if(chest != null || chest == null && TNE.instance().api().getBoolean("Signs.Item.EnderChest")) {
      if(chest == null) chest = new SignChest();
      if(!chest.isEnder() || chest.isEnder() && TNE.instance().api().getBoolean("Signs.Item.EnderChest")) {
        BlockState state = chest.getLocation().getBlock().getState();
        Inventory inventory = (chest.isEnder())? IDFinder.getOffline(owner).getPlayer().getEnderChest()
        ...
        TNE.debug("Item Count: " + itemCount);
        if (add) {
          int leftOver = MISCUtils.leftOver(inventory, material, itemCount + amount);
          if(leftOver > 0) {
            if(Bukkit.getOnlinePlayers().contains(IDFinder.getOffline(owner).getPlayer())) {
        ...
        MISCUtils.setItemCount(inventory, material, itemCount - amount);
        if(chest.isEnder()) {
          IDFinder.getOffline(owner).getPlayer().getEnderChest().setContents(inventory.getContents());
        } else {
  ...
  public boolean onClick(Player player, boolean shift) {
    ...
    if(super.onClick(player, shift)) {
      if (player.hasPermission(SignType.ITEM.getUsePermission())) {
        if(shift && TNE.instance().api().getBoolean("Signs.Item.Multiple", world, IDFinder.getID(player))) {
          if(!IDFinder.getID(player).equals(owner)) {
            new Message("Messages.General.NoPerm").translate(world, player);
            return false;
          }
